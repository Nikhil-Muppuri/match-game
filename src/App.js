...
class App extends Component {
  ...
  tick = () => {
    ...
    if (time === 0) {
      this.setState({time: 0, isProgress: false})
    }
  }

  onChangeTab = tabId => {
    this.setState({tabOption: tabId})
  }
  ...
  onClickedImage = id => {
    ...
    const finalOne = imagesList[number].id === id
    ...
    if (finalOne) {
      ...
      this.setState({isProgress: false, time})
    }
  }
    ...
    return (
      <div className="bg2">
        <div className="game-over-bg">
          ...
          <p className="final-score-m">{score}</p>
  ...
  renderInitialPage = () => {
    const {tabOption, number} = this.state

    const filteredResult = imagesList.filter(
      eachOne => eachOne.category === tabOption,
    )
    ...
    return (
      <div className="bg2">
        <center>
          <img
            src={imagesList[number].imageUrl}
        ...
        <ul className="ul-container">
          {tabsList.map(eachTab => (
            <TabsList
              ...
              isActive={eachTab.tabId === tabOption}
            />
          ))}
        </ul>

        <ul>
          {filteredResult.map(eachImage => (
            <ImageItem
  ...
  render() {
    const {time, isProgress, score} = this.state

    let final

    if (isProgress) {
      final = this.renderInitialPage()
    } else {
      final = this.renderGameOver()
    }
